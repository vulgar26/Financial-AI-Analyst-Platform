...
> 上面这些都是**被测系统自己产出**的 `error_code`（响应顶层字段）——它是被测系统对自身行为的**自我表征（供词）**，由被测系统拥有与迭代。它与判定器的「判词」是两个世界的东西，别混。下一节说明这条边界。

## error_code 比对：供词 vs 判词（跨 eval 项目）

数据集用例可选带一个 `expected_error_code` 字段（schema 见 `DatasetDrivenSafetyEvalTest` 与 `eval/finance-safety-p0.jsonl`）。它表达「这条用例期望被测系统报出哪个供词」。判定流程在 eval 项目（`eval` 仓的 `RunEvaluator`）里：行为（behavior）匹配后，**若**用例声明了 `expected_error_code`，再把它与被测响应顶层 `error_code` 做大小写/空白归一比较。

**两个 error_code 归属不同，必须解耦：**

| | 来源 | 性质 | 谁拥有 |
| --- | --- | --- | --- |
| 响应顶层 `error_code`（如 `PROMPT_INJECTION_BLOCKED`） | 被测系统产出 | **供词**——被测系统对自身行为的自我表征 | 被测系统（本仓），可自行迭代 |
| `RunModel.ErrorCode` 枚举（含 `ERROR_CODE_MISMATCH`） | 判定器产出 | **判词**——判定器对一次评测的裁决，是打分契约的一部分 | 判定器（`eval` 仓） |

比对结果落进判定证据：匹配则 `error_code_match=true`；不匹配则判定 `FAIL` 且裁决码 `ERROR_CODE_MISMATCH`（连同 `expected_error_code`/`actual_error_code` 入 debug 证据，经 `DebugSanitizer` 白名单放行后进报告，否则会成为「看不见比对结果」的说谎绿）。

**注意一个跨项目同步面**：本表「安全用例」里 error_code 列的值（供词）若改名，数据集里的 `expected_error_code` 期望要跟着改，否则判定器会诚实报 `ERROR_CODE_MISMATCH`——这个红是**特性**（守住了契约），但要分清是「行为退化」还是「仅改名」。详见 eval 仓的判定器实现与 `RunEvaluatorTest`。
